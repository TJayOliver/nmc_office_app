...
// render request for certificate forms (GET)
export const requestForCertificateView = (req,res)=>{
    const staffName = req.user.name;
    res.render('forms/requestforcertificate', {PageTitle: "REQUEST FOR CERTIFICATE", action:'ADD', staffName:staffName, req:req})
}

// render request for certificate forms (POST)
export const requestForCertificateForm = async(req,res)=>{
    const {registrationNumber, requestType, amountPaid} = req.body; 
    ...
    try{
        console.log('request for certificate Forms Connection Successful');
        ...
        if(data.length > 0){
            ...
            const sqlquery = `INSERT INTO request_for_certificate 
            SET 
            registration_number=?,  
            request_type=?,
            amount_paid=?,
            attended_to_by_client_service=?`;
            const parameters = [registrationNumberr, requestType, amountPaid, attended_to_by_client_service];
            const [data] = await database.query(sqlquery, parameters)
            req.flash('Success', 'SAVED! ALL GOOD!'); 
            res.redirect('/request-for-certificate')
        }else{
...
// render request for certificate edit forms
export const requestForCertificateEdit = async(req, res)=>{
    try{
        console.log('EDIT request for certificate forms successful')
        const editQuery = 'SELECT * FROM request_for_certificate WHERE id=?', editParameters = [req.params.id];
        const [data] = await database.query(editQuery, editParameters)
        const staffName = req.user.name;
        res.render('forms/requestforcertificate', {PageTitle: "REQUEST FOR CERTIFICATE", action:'EDIT', data:data, staffName:staffName, req:req})
    }catch(error){
        console.log(error)
        res.status(500).render('errors/servererror')
    }
...
// update request for certificate edit forms
export const requestForCertificateUpdate = async(req,res)=>{
    console.log('request for certificate Update Connection Succcessful');
    const {registrationNumber, requestType, amountPaid, bank, receiptNumber, transactionRef} = req.body; 
    ...
    try{
        ...
        const CheckQuery = `SELECT * FROM request_for_certificate WHERE attended_to_by_account_office=?`;
        ...
        connection.query(CheckQuery,[attended_to_by_account_office])
        // updated by 
        if(data.length > 0){
            const updateQuery = `UPDATE request_for_certificate 
            SET 
            registration_number=?,  
            request_type=?,
            amount_paid=?,
            ...
            WHERE id=?`;
            const updateParameter = [registrationNumberr, requestType, amountPaid, bank, receiptNumber, transactionReff, attended_to_by_account_office, modified_by_client_service,modified_by_account_office, req.params.id];
            const [data] = await database.query(updateQuery, updateParameter)
            req.flash('Updated', 'SUCCESSFULLY UPDATED');
            res.redirect('/records/request-for-certificate')
        }else{
            // new insertion
            const updateQuery = `UPDATE request_for_certificate 
            SET 
            ...
            modified_by_client_service=?
            WHERE id=?`;
            const updateParameter = [registrationNumberr, requestType, amountPaid,  bank, receiptNumber, transactionReff, attended_to_by_account_office, modified_by_client_service, req.params.id];
            const [data] = await database.query(updateQuery, updateParameter)
            req.flash('Updated', 'SUCCESSFULLY UPDATED');
            res.redirect('/records/request-for-certificate')
        }
...
// delete record from request for certificate form
export const requestForCertificateDelete = async(req,res)=>{
    try{
        console.log('delete request for certificate connection successfull');
        const deleteQuery = 'DELETE FROM request_for_certificate WHERE id=?', deleteParameter = [req.params.id];
...
export const requestForCertificateExport = async(req, res) =>{
    try{
        ...
        res.setHeader("Content-Disposition", "attachment; filename=REQUEST FOR CERTIFICATE.csv");
