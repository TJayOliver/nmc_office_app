...
// render Request for Logbook forms (GET)
export const requestForLogbookView = (req,res)=>{
    const staffName = req.user.name;
    res.render('forms/requestforlogbook', {PageTitle: "REQUEST FOR LOGBOOK", action:'ADD', staffName:staffName, req:req})
}

// render Request for Logbook forms (POST)
export const requestForLogbookForm = async(req,res)=>{
    ...
    try{
        console.log('Request for Logbook Forms Connection Successful');
        ...
        if(data.length > 0){
            const sqlquery = `INSERT INTO request_for_logbook 
            SET 
            registration_number=?,  
            amount_paid=?,
            ...
            res.redirect('/request-for-logbook')
        }
...
// render Request for Logbook edit forms
export const requestForLogbookEdit = (req, res)=>{
    try{
        console.log('EDIT Request for Logbook forms successful')
        const editQuery = 'SELECT * FROM request_for_logbook WHERE id=?', editParameters = [req.params.id];
        connection.query(editQuery, editParameters)
        const staffName = req.user.name;
        res.render('forms/requestforlogbook', {PageTitle: "REQUEST FOR LOGBOOK", action:'EDIT', data:data, staffName:staffName, req:req})
    }catch(error){
        console.log(error)
        res.status(500).render('errors/servererror')
    }
...
// update Request for Logbook edit forms
export const requestForLogbookUpdate = async(req,res)=>{
    ...
    try{
        ...
        const CheckQuery = `SELECT * FROM request_for_logbook WHERE attended_to_by_account_office=?`;
        ...
        // updated by 
        if(data.length > 0){
            const updateQuery = `UPDATE request_for_logbook 
            SET 
            registration_number=?,  
            amount_paid=?,
            bank=?,
            ...
            res.redirect('/records/request-for-logbook')
        }else{
            // new insertion
            const updateQuery = `UPDATE request_for_logbook 
            SET 
...
// delete record from Request for Logbook form
export const requestForLogbookDelete = async(req,res)=>{
    try{
        console.log('delete Request for Logbook connection successfull');
        const deleteQuery = 'DELETE FROM request_for_logbook WHERE id=?', deleteParameter = [req.params.id];
...
export const requestForLogbookExport = async(req, res) =>{
    try{
        ...
        res.setHeader("Content-Disposition", "attachment; filename=REQUEST FOR LOGBOOK.csv");
